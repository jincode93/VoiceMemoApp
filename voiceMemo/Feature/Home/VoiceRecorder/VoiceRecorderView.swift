...
private struct VoiceRecorderCellView: View {
    ...
    private var progressBarValue: Float {
        if voiceRecorderViewModel.selectedRecordedFile == recorderFile
    ...
    fileprivate init(
        ...
        self.voiceRecorderViewModel = voiceRecorderViewModel
        self.recorderFile = recorderFile
        (self.creationDate, self.duration) = voiceRecorderViewModel.getFileInfo(for: recorderFile)
    }
    ...
    fileprivate var body: some View {
        VStack {
            Button(
                action: {
                    voiceRecorderViewModel.voiceRecordCellTapped(recorderFile)
                },
                label: {
                    VStack {
                        HStack {
                            Text(recorderFile.lastPathComponent)
                        ...
                        Button(
                            action: {
                                if voiceRecorderViewModel.isPaused {
                                    ...
                                    voiceRecorderViewModel.startPlaying(recordingURL: recorderFile)
                                }
                            }, 
                            label: {
                                Image("play")
